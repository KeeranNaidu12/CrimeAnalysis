...
TABLE_NAME = "csi_assault"  # Changed to assault table

OUT_DIR  = Path("project/outputs")  # Separate output directory for assault
...
LGB_REG_LAMBDA       = 1.0
LGB_RANDOM_STATE     = 42
ASSAULT_BOOST        = 1.5  # Boost for assault recall (higher than collisions)

# ── Hyperparameter Tuning & Optimization ─────────────────────────────────────
TUNE_ENABLED    = True
TUNE_ITERATIONS = 50
TUNE_OBJECTIVE  = "recall_1"   # "recall_1", "f1_1", or "composite"
MIN_RECALL_0    = 0.55          # Minimum acceptable non-assault recall floor
TUNE_THRESHOLD  = 0.45          # Threshold used during tuning (biases toward assault)
...
# Feature columns for assault prediction (similar structure to collisions)
FEATURE_COLS = [
    ...
    "ewma_3w", "rate_change_3w", "assault_trend",
    "recent_assault_density", "seasonal_pattern",
...
FEATURE_DISPLAY_NAMES = [
    ...
    "EWMA 3w", "Rate Change 3w", "Assault Trend",
...
def load_and_preprocess(engine):
    ...
    print(f"    Loaded {len(df):,} assault records")
    ...
    # Optional: Filter for Assault category if multiple CSI categories exist
    if 'csi_category' in df.columns:
        assault_count = len(df[df['csi_category'] == 'Assault'])
        print(f"    Assault records: {assault_count:,} ({assault_count/len(df)*100:.1f}%)")
        # Keep all records but mark assault occurrences
    ...
    for start in date_range:
        ...
        for neigh in neighbourhoods:
            ...
            records.append({
                ...
                "season_num":    seas_val,
                "holiday":       hol_val,
                "assault":       label,  # Changed from collision to assault
                "year":          start.year,
            })

    result = pd.DataFrame(records)
    print(f"    {len(result):,} samples. Assault rate: {result['assault'].mean():.1%}")
    return result
...
def engineer_features(windows_df):
    ...
    city_rates = df.groupby('window_start')['assault'].mean()
    ...
    for neigh, grp in df.groupby('neighbourhood'):
        idx = grp.index
        assault_vals = grp['assault'].values.astype(float)
        n = len(assault_vals)

        past = np.full(n, np.nan)
        past[1:] = assault_vals[:-1]
        ...
        df.loc[idx, 'recent_assault_density'] = s_past.rolling(2, min_periods=1).sum().values

        seasonal = np.full(n, 0.0)
        if n > 72:
            seasonal[72:] = assault_vals[:-72]
        ...
        df.loc[idx, 'assault_trend'] = slopes
...
def encode_and_split(windows_enriched):
    ...
    y = windows_enriched["assault"]  # Changed from collision to assault
...
def tune_lgbm(X_train, y_train, X_val, y_val):
    ...
    n_neg = (y_train == 0).sum()
    n_pos = (y_train == 1).sum()
    scale_w = (n_neg / max(n_pos, 1)) * ASSAULT_BOOST
...
def evaluate(clf, X_val, y_val):
    ...
    print("[7/8] Evaluating & Optimizing Threshold (Priority: Assault Recall) ...")
    ...
    print("="*55)
    print(classification_report(y_val, y_pred, target_names=["No Assault", "Assault"]))
    ...
    fig.suptitle(f"Assault Prediction — {val_label} Validation", fontsize=15, fontweight="bold")
    ConfusionMatrixDisplay(confusion_matrix(y_val, y_pred),
                           display_labels=["No Assault", "Assault"]).plot(
    ...
    importances.plot(kind="barh", ax=ax3, color="#D62728", edgecolor="white")
...
def run_shap(clf, X_train, X_val):
    ...
    plt.title("SHAP Summary — Impact on Assault Probability", fontsize=12, pad=10)
    ...
    ax_d.scatter(val_sample.iloc[:, top_idx].values, shap_pos[:, top_idx],
                 alpha=0.5, edgecolors="none", color="#D62728", s=20)
...
def predict_window(clf, le, start_date, windows_enriched, threshold=0.5):
    ...
    pred_df["assault_probability"] = probs
    pred_df["predicted_assault"]   = (probs >= threshold).astype(int)

    top = pred_df.sort_values("assault_probability", ascending=False).head(TOP_N)
    print(f"\n  Top {TOP_N} highest-risk neighbourhoods for Assault:")
    print(f"  {'Neighbourhood':<45} {'Prob':>6}  {'Assault?':>10}")
    print(f"  {'-'*65}")
    for _, row in top.iterrows():
        flag = "RED YES" if row["predicted_assault"] else "GREEN NO"
        print(f"  {row['neighbourhood']:<45} {row['assault_probability']:>5.1%}  {flag:>10}")
...
def main():
    ...
    try:
        ...
        model_path = OUT_DIR / "assault_model_bundle.joblib"
        joblib.dump(model_bundle, model_path)
        print(f"\n✅ Model bundle saved to: {model_path}")
        print("   To load in another app:")
        print("   >>> import joblib")
        print("   >>> from pathlib import Path")
        print("   >>> bundle = joblib.load(Path('path/to/assault_model_bundle.joblib'))")
        ...
        print("  assault_model_bundle.joblib  ← Load this in other apps!")
