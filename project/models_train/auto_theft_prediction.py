...
Just edit the CONFIG section below, then run:
    ...
    python auto_theft_prediction_lgbm.py
"""
...
LGB_REG_LAMBDA       = 1.0
LGB_RANDOM_STATE     = 42
AUTO_THEFT_BOOST        = 1.5  # Boost for auto theft recall

# ── Hyperparameter Tuning & Optimization ─────────────────────────────────────
TUNE_ENABLED    = True
TUNE_ITERATIONS = 50
TUNE_OBJECTIVE  = "recall_1"   # "recall_1", "f1_1", or "composite"
MIN_RECALL_0    = 0.55          # Minimum acceptable non-auto_theft recall floor
...
FEATURE_COLS = [
    ...
    "ewma_3w", "rate_change_3w", "auto_theft_trend",
    "recent_auto_theft_density", "seasonal_pattern",
...
def load_and_preprocess(engine):
    ...
    print(f"    Loaded {len(df):,} auto theft records")
    ...
    # Optional: Filter for Auto Theft category if multiple CSI categories exist
    if 'csi_category' in df.columns:
        auto_theft_count = len(df[df['csi_category'] == 'Auto Theft'])
        print(f"    Auto Theft records: {auto_theft_count:,} ({auto_theft_count/len(df)*100:.1f}%)")
        # Keep all records but mark auto theft occurrences
    ...
    for start in date_range:
        ...
        for neigh in neighbourhoods:
            ...
            records.append({
                ...
                "season_num":    seas_val,
                "holiday":       hol_val,
                "auto_theft":    label,  # Changed from collision/assault to auto theft
                "year":          start.year,
            })

    result = pd.DataFrame(records)
    print(f"    {len(result):,} samples. Auto Theft rate: {result['auto_theft'].mean():.1%}")
    return result
...
def engineer_features(windows_df):
    ...
    city_rates = df.groupby('window_start')['auto_theft'].mean()
    ...
    for neigh, grp in df.groupby('neighbourhood'):
        idx = grp.index
        auto_theft_vals = grp['auto_theft'].values.astype(float)
        n = len(auto_theft_vals)

        past = np.full(n, np.nan)
        past[1:] = auto_theft_vals[:-1]
        ...
        df.loc[idx, 'recent_auto_theft_density'] = s_past.rolling(2, min_periods=1).sum().values

        seasonal = np.full(n, 0.0)
        if n > 72:
            seasonal[72:] = auto_theft_vals[:-72]
        ...
        df.loc[idx, 'auto_theft_trend'] = slopes
...
def encode_and_split(windows_enriched):
    ...
    y = windows_enriched["auto_theft"]  # Changed from collision/assault to auto theft
...
def tune_lgbm(X_train, y_train, X_val, y_val):
    ...
    n_neg = (y_train == 0).sum()
    n_pos = (y_train == 1).sum()
    scale_w = (n_neg / max(n_pos, 1)) * AUTO_THEFT_BOOST
...
def predict_window(clf, le, start_date, windows_enriched, threshold=0.5):
    ...
    pred_df["auto_theft_probability"] = probs
    pred_df["predicted_auto_theft"]   = (probs >= threshold).astype(int)

    top = pred_df.sort_values("auto_theft_probability", ascending=False).head(TOP_N)
    print(f"\n  Top {TOP_N} highest-risk neighbourhoods for Auto Theft:")
    print(f"  {'Neighbourhood':<45} {'Prob':>6}  {'Auto Theft?':>10}")
    print(f"  {'-'*65}")
    for _, row in top.iterrows():
        flag = "RED YES" if row["predicted_auto_theft"] else "GREEN NO"
        print(f"  {row['neighbourhood']:<45} {row['auto_theft_probability']:>5.1%}  {flag:>10}")
...
def main():
    ...
    try:
        ...
        model_path = OUT_DIR / "auto_theft_model_bundle.joblib"
        joblib.dump(model_bundle, model_path)
        print(f"\n✅ Model bundle saved to: {model_path}")
        print("   To load in another app:")
        print("   >>> import joblib")
        print("   >>> from pathlib import Path")
        print("   >>> bundle = joblib.load(Path('path/to/auto_theft_model_bundle.joblib'))")
        ...
        print("  auto_theft_model_bundle.joblib  ← Load this in other apps!")
